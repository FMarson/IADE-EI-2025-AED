...
namespace sortings
{
	...
	public:
		...
		static void sort( RandomAccessIterator first, RandomAccessIterator last, Compare comp )
		{
			...
			std::vector< typename std::iterator_traits<RandomAccessIterator>::value_type > data( size );

			for( auto sz = 1; sz < size; sz += sz )
				for( auto lo = 0; lo < size - sz; lo += sz+sz )
					merge( first, last, data.begin(), lo, lo+sz - 1, std::min<long>(lo+sz+sz-1, size-1), comp );
		}
